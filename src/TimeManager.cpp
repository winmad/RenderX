...
double TimeManager::PopCurrentTime() {
...
double TimeManager::GetElapsedTime(int id) {
    double start = stopWatches[id].startTime;
    double finish = getRealTime();
    return finish - start;
}

void TimeManager::DisplayElapsedTime(int id, string textFormat){
	printf(textFormat.c_str(), GetElapsedTime(id));
#ifdef _WIN32
	flushall();
#endif
}
