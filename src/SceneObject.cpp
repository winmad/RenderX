...
Ray SceneObject::scatter(const Ray& inRay, const bool fixIsLight, const bool russian) const
{
	if(materialList.size() && this)
	{
		return materialList[rand()%materialList.size()]->scatter(this, inRay, fixIsLight, russian);
	}
	return Ray();
}